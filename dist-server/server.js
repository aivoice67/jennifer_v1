...
let devViteServer; // keep reference for dev index transform
...
function buildSystemPrompt(language, assessments) {
    const prompts = {
        english: {
...
app.post('/api/chat', async (req, res) => {
    ...
    try {
        if (params.FirstMessage) {
            ...
            const payload = { audioData: audioB64, text: aiText };
...
async function addSpaFallback(app) {
    if (isProduction) {
        app.get('*', (_req, res) => {
